...
export default {
  ...
  methods: {
    ...
    save(item) {
      ...
      axios.put('http://localhost:1600/products/users/bets', this.userBets)
      this.close()
    },
    async dessertsIt() {
      this.emailUser.email = localStorage.email
      await axios.post('http://localhost:1600/products/get/users/history', this.emailUser).then(response_history => {
